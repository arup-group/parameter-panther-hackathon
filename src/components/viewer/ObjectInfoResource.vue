  ...
  <div>
    <v-card class="mx-2 mb-2 rounded-lg">
      ...
      >
        ...
        <v-chip small class="ma-1 caption grey white--text no-hover"
          >Object</v-chip
        >
        <v-spacer />
        <v-btn
...
export default {
  components: {
    ObjectProperties: () => import("./ObjectProperties"),
  },
  props: {
    resource: {
      type: Object,
  ...
  computed: {
    ...
    visible() {
      return (
        this.$store.state.hideValues.indexOf(this.resource.data.object.id) ===
        -1
      );
    },
  },
